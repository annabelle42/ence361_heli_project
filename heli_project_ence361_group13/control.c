...
void flyingFSM (int modeButton)
{
    ...
    if (meanAlt > 7 && currentStatus[TAKING_OFF] && currentStatus[FINISHED_SORTIE])
    {
        currentStatus[TAKING_OFF] = 0; //takingoff = 0;
        currentStatus[FLYING] = 1; //flying = 1;
    }
    else if (meanAlt < 3 && currentStatus[LANDING])
    {
        currentStatus[LANDING] = 0; //landing = 0;
        currentStatus[LANDED] = 1; //landed = 1;
    }
    ...
    {
    case PUSHED:
        if (!currentStatus[TAKING_OFF] && !currentStatus[LANDING])
        {
            currentStatus[LANDED] = 0; //landed = 0;
            currentStatus[LANDING] = 0; //landing = 0;
            currentStatus[TAKING_OFF] = 1; //takingoff = 1;
            currentStatus[FLYING] = 0; //flying = 0;
            desiredAltitude = 10;
            currentStatus[FINISHED_SORTIE] = 0; //finishedSortie = 0;
        }
        break;
    case RELEASED:
        if (!currentStatus[TAKING_OFF] && !currentStatus[LANDING])
        {
            currentStatus[LANDED] = 0; //landed = 0;
            currentStatus[LANDING] = 1; //landing = 1;
            currentStatus[TAKING_OFF] = 0; //takingoff = 0;
            currentStatus[FLYING] = 0; //flying = 0;
            desiredAltitude = 0;
        }
...
void sortie (void)
{
    ...
    {
        setPWMTail(200, 0);
        status[FINISHED_SORTIE] = 1; //finishedSortie = 1;
    }
}
