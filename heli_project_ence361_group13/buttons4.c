...
// Globals to module
// *******************************************************
static bool but_state[NUM_BUTTONS];	// Corresponds to the electrical state
static uint8_t but_count[NUM_BUTTONS];
static bool but_flag[NUM_BUTTONS];
static bool but_normal[NUM_BUTTONS];   // Corresponds to the electrical state
...
initButtons (void)
{
    ...
    // SELECT switch
    SysCtlPeripheralEnable (SELECT_BUT_PERIPH);
    GPIOPinTypeGPIOInput (SELECT_BASE, SELECT_PIN);
    but_normal[SELECT] = 0; //normal state is down
...
updateButtons (void)
{
    ...
    but_value[SELECT] = (GPIOPinRead (SELECT_BASE, SELECT_PIN) == SELECT_PIN);
